...
function validate(source, opts, cb) {
  ...
  var series = [],
    k, z, matcher, matchtmp, fields = this.rules.fields,
    state = opts.state || {}
  ...
  for (k in fields) {
    if (typeof fields[k] === 'object' && (fields[k].match instanceof RegExp)) {
      matcher = fields[k];
      delete fields[k];
      for (z in source) {
  ...
  function getRule(rule, field, value) {
    ...
    if (value === undefined && typeof rule.placeholder === 'function') {
    ...
    if (typeof(rule.test) !== 'function' && Array.isArray(rule.type)) {
      rule.test = Rule.types;
    }
    if (typeof rule === 'function') {
      if (rule.isSchemaFunction) {
        return getRule(rule.call(self, rule, value), field, value);
      }
      rule.test = rule;
  ...
  func(series, function(rule, callback) {
    ...
    isDeep = (rule.type === 'object' || rule.type === 'array') && typeof(rule.fields) === 'object';
    ...
    function onValidate(err) {
      ...
      if (!isDeep) {
        ...
        func(keys, function iterator(key, cb) {
          ...
          if (descriptor.type === 'array' && typeof descriptor.values === 'object') {

            // wrap objects as arrays
            len = Array.isArray(descriptor.values) ? descriptor.values.length : Array.isArray(value) ? value.length : 0;
...
function complete(err, results, opts, callback) {
  ...
  callback(err, !errors.length ? null : {
    errors: errors,
    fields: fields
  });
}
...
function clone(source, target) {
  ...
  function isComplex(obj) {
    return Array.isArray(obj) || (obj && typeof obj === 'object') && !(obj instanceof RegExp);
  }
...
Schema.Rule = Rule; //export Rule as well.
